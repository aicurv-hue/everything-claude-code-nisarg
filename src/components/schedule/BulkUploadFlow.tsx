...
function downloadTemplate() {
  ...
  a.href = url; a.download = "cridl-bulk-template.csv"; a.click();
...
function ValidationCell({ value, error }: { value: string; error?: string }) {
  if (!error) {
    return <span className="text-xs text-[var(--foreground)]">{value || <span className="text-[var(--text-muted)] italic">—</span>}</span>;
  }
  return (
    <div>
      <span className="text-xs text-red-400 font-medium">{value || "missing"}</span>
...
export default function BulkUploadFlow({ segment, onComplete, onViewCalendar }: Props) {
  ...
  const accent     = isCorp ? "bg-violet-600 hover:bg-violet-700" : "bg-[var(--primary)] hover:opacity-90";
  const accentText = isCorp ? "text-violet-400" : "text-[var(--primary)]";
  ...
  if (step === 1) return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-[var(--foreground)]">Bulk Schedule via CSV</h2>
          <p className="text-sm text-[var(--text-muted)] mt-0.5">Plan weeks of content at once — up to 500 posts per file</p>
        </div>
        <button
          onClick={downloadTemplate}
          className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[var(--border)] bg-[var(--card)] hover:bg-[var(--card-hover)] text-sm font-medium text-[var(--foreground)] transition-all"
        >
      ...
      <div className="card p-5 space-y-4">
        <div className="flex items-start gap-2">
          <div>
            <p className="text-xs font-semibold text-[var(--foreground)] mb-0.5">How it works</p>
            <p className="text-[11px] text-[var(--text-muted)] leading-relaxed">
              Fill in your topics and schedule dates. Cortex will generate the post text automatically at publish time.
              If you already have written posts, paste them in the <code className="bg-[var(--toggle-bg)] px-1 rounded">content</code> column — they'll be used as-is.
            </p>
          </div>
        </div>

        <div className="border-t border-[var(--border-sub)] pt-4">
          <p className="text-[11px] font-semibold text-[var(--text-muted)] uppercase tracking-wide mb-3">Column reference</p>
          <div className="grid grid-cols-2 gap-x-6 gap-y-2.5">
            {[
              ...
              <div key={col} className="flex items-start gap-2">
                <div className={`mt-0.5 shrink-0 text-[10px] font-bold px-1.5 py-0.5 rounded ${req ? "bg-red-100 text-red-400" : "bg-[var(--toggle-bg)] text-[var(--text-muted)]"}`}>
                  {req ? "REQ" : "OPT"}
                </div>
                <div>
                  <div className="flex items-center gap-1">
                    <code className="text-xs font-mono font-semibold text-[var(--foreground)]">{col}</code>
                    <HelpTooltip text={help} position="right" width="w-72" />
                  </div>
                  <p className="text-[10px] text-[var(--text-muted)] leading-tight">{note}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-blue-500/10 border border-blue-100 rounded-lg px-3 py-2.5 text-[11px] text-blue-400 leading-relaxed">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all ${
          isDragging
            ? (isCorp ? "border-violet-400 bg-violet-50" : "border-[#0A66C2] bg-blue-500/10")
            : "border-[var(--border)] bg-[var(--card-hover)] hover:border-slate-300 hover:bg-[var(--card)]"
        }`}
      >
        ...
        <div className="flex flex-col items-center gap-3">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isCorp ? "bg-violet-100" : "bg-blue-100"}`}>
            <Upload className={`w-6 h-6 ${isCorp ? "text-violet-400" : "text-[var(--primary)]"}`} />
          </div>
          <div>
            <p className="text-sm font-semibold text-[var(--foreground)]">Drop your CSV here, or click to browse</p>
            <p className="text-xs text-[var(--text-muted)] mt-1">Supports .csv files up to 5 MB · max 500 rows</p>
          </div>
        </div>
      </div>
      ...
      {fileError && (
        <div className="flex items-start gap-2 px-4 py-3 rounded-xl bg-red-50 border border-red-200">
          ...
          <div>
            <p className="text-sm font-semibold text-red-400">File Error</p>
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          <button onClick={reset} className="w-8 h-8 rounded-lg hover:bg-[var(--toggle-bg)] flex items-center justify-center transition-all">
            <ArrowLeft className="w-4 h-4 text-[var(--text-muted)]" />
          </button>
          <div>
            <h2 className="text-lg font-bold text-[var(--foreground)]">Review & Validate</h2>
            <p className="text-sm text-[var(--text-muted)]">{fileName} — {validations.length} rows found</p>
          </div>
        </div>

        {/* Summary bar */}
        <div className="flex gap-3 flex-wrap">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-800/40 text-xs font-semibold text-emerald-400">
            <CheckCircle className="w-3.5 h-3.5" /> {validCount} ready to schedule
          </div>
          {invalidCount > 0 && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-50 border border-red-200 text-xs font-semibold text-red-400">
              <AlertCircle className="w-3.5 h-3.5" /> {invalidCount} rows have errors — fix in CSV and re-upload
            </div>
          )}
          {invalidCount === 0 && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-500/10 border border-blue-800/40 text-xs font-semibold text-[var(--primary)]">
        ...
        {invalidCount > 0 && (
          <div className="flex items-start gap-2 px-4 py-3 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-400">
        ...
        <div className="card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-[var(--border-sub)] bg-[var(--card-hover)] text-[10px] uppercase tracking-wide text-[var(--text-muted)] font-semibold">
                ...
                {displayRows.map((v) => (
                  <tr
                    ...
                    className={`transition-colors ${
                      v.isValid ? "hover:bg-[var(--card-hover)]" : "bg-red-50/40 hover:bg-red-50"
                    }`}
                  >
                    {/* Row number */}
                    <td className="px-3 py-2.5 text-[11px] text-[var(--text-muted)] font-mono">{v.row}</td>
                    ...
                    <td className={`px-3 py-2.5 ${v.errors.tone ? "bg-red-100/60" : ""}`}>
                      {v.errors.tone ? (
                        ...
                        <span className={`text-[11px] px-2 py-0.5 rounded capitalize font-medium ${
                          v.tone.startsWith("(") ? "text-[var(--text-muted)]" : "bg-blue-500/10 text-blue-400"
                        }`}>
                          {v.tone}
                        </span>
                      )}
                    ...
                    <td className={`px-3 py-2.5 ${v.errors.length ? "bg-red-100/60" : ""}`}>
                      {v.errors.length ? (
                        ...
                        <span className={`text-[11px] px-2 py-0.5 rounded capitalize ${
                          v.length.startsWith("(") ? "text-[var(--text-muted)]" : "bg-[var(--toggle-bg)] text-[var(--text-sub)]"
                        }`}>
                          {v.length}
                        </span>
                      )}
                    ...
                    <td className={`px-3 py-2.5 max-w-[200px] ${v.errors.content ? "bg-red-100/60" : ""}`}>
                      {v.errors.content ? (
                        ...
                        <span className={`text-[11px] ${v.content === "(Cortex generates this)" ? "text-[var(--text-muted)] italic" : "text-[var(--text-sub)]"}`}>
                          {v.content}
                        </span>
                      )}
                    </td>
          ...
          {validations.length > 20 && (
            <div className="border-t border-[var(--border-sub)] px-4 py-3 text-center">
              <button
                onClick={() => setShowAllRows(!showAllRows)}
                className="text-xs text-[var(--text-muted)] hover:text-[var(--foreground)] transition-colors font-medium"
              >
                {showAllRows
                  ? "Show fewer rows"
        ...
        <div className="card p-4 bg-[var(--card-hover)] border-[var(--border-sub)]">
          <p className="text-[11px] font-semibold text-[var(--text-muted)] uppercase tracking-wide mb-2">Valid values reference</p>
          <div className="grid grid-cols-3 gap-3 text-[11px]">
            <div>
              <p className="font-semibold text-[var(--text-sub)] mb-1">tone</p>
              {VALID_TONES.map(t => <p key={t} className="text-[var(--text-muted)]">• {t}</p>)}
            </div>
            <div>
              <p className="font-semibold text-[var(--text-sub)] mb-1">length</p>
              {VALID_LENGTHS.map(l => <p key={l} className="text-[var(--text-muted)]">• {l}</p>)}
            </div>
            <div>
              <p className="font-semibold text-[var(--text-sub)] mb-1">date / time format</p>
              <p className="text-[var(--text-muted)]">date: 22-Mar-27</p>
              <p className="text-[var(--text-muted)]">time: 09:30 or 17:00</p>
              <p className="text-[var(--text-muted)] mt-1 text-[10px]">DD-Mon-YY · 24-hour · future only</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={reset}
            className="flex-1 py-2.5 rounded-lg border border-[var(--border)] text-sm font-medium text-[var(--text-sub)] hover:bg-[var(--card-hover)] transition-all"
          >
            ← Re-upload CSV
          </button>
          <button
  ...
  return (
    <div className="flex flex-col items-center py-12 text-center space-y-5">
      ...
      <div>
        <h2 className="text-xl font-bold text-[var(--foreground)]">Upload Complete!</h2>
      ...
      <div className="max-w-sm text-[12px] text-[var(--text-muted)] bg-[var(--card-hover)] border border-[var(--border-sub)] rounded-xl px-4 py-3 text-left space-y-1.5">
      ...
      {(result?.errors || 0) > 0 && (
        <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-amber-50 border border-amber-200 text-sm text-amber-400 max-w-sm">
      ...
      <div className="flex gap-3 pt-2">
        <button
          onClick={reset}
          className="flex items-center gap-2 px-5 py-2.5 rounded-lg border border-[var(--border)] text-sm font-medium text-[var(--foreground)] hover:bg-[var(--card-hover)] transition-all"
        >
