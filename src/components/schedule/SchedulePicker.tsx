...
export default function SchedulePicker({ onSchedule, onCancel, isLoading, userId = "demo-user", segment = "individual", initialDate }: Props) {
  ...
  const accent      = isCorporate ? "bg-violet-600 hover:bg-violet-700" : "bg-[var(--primary)] hover:opacity-90";
  ...
  const inputClass = `w-full bg-[var(--card)] border border-[var(--border)] rounded-lg px-3 py-2.5 text-sm text-[var(--foreground)] focus:outline-none focus:ring-2 transition-all ${focusRing}`;
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end md:items-center justify-center md:p-4" onClick={(e) => e.target === e.currentTarget && onCancel()}>
      <div className="bg-[var(--card)] rounded-t-2xl md:rounded-2xl shadow-2xl w-full md:max-w-md max-h-[92dvh] overflow-y-auto modal-enter">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-sub)]">
          <div className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isCorporate ? "bg-violet-100" : "bg-blue-100"}`}>
              <CalendarDays className={`w-4 h-4 ${isCorporate ? "text-violet-400" : "text-[var(--primary)]"}`} />
            </div>
            <div>
              <p className="text-sm font-semibold text-[var(--foreground)]">Schedule Post</p>
              <p className="text-[11px] text-[var(--text-muted)]">Pick a date & time</p>
            </div>
          </div>
          <button onClick={onCancel} className="w-7 h-7 rounded-lg hover:bg-[var(--toggle-bg)] flex items-center justify-center transition-all">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {/* Timezone */}
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[var(--card-hover)] border border-[var(--border-sub)]">
            <Globe className="w-3.5 h-3.5 text-[var(--text-muted)] shrink-0" />
            <span className="text-xs text-[var(--text-muted)] truncate">{timezone}</span>
          </div>
        ...
        {submitError && (
          <div className="px-6 pb-2">
            <p className="text-xs text-red-400 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{submitError}</p>
          </div>
        )}
        <div className="px-6 py-4 border-t border-[var(--border-sub)] flex gap-3">
          <button
            onClick={onCancel}
