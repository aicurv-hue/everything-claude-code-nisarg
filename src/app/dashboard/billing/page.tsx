...
function UsageBar({ label, used, limit, tooltip, displayUnlimited }: { label: string; used: number; limit: number; tooltip?: string; displayUnlimited?: boolean }) {
  ...
  const color = !isUnlimited && pct >= 90 ? "bg-red-500/100" : !isUnlimited && pct >= 70 ? "bg-amber-400" : "bg-[var(--primary)]";
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between text-sm">
        <span className="text-[var(--foreground)] font-medium flex items-center gap-1.5">
          ...
          {tooltip && (
            <span className="group relative inline-flex items-center">
              <span className="w-3.5 h-3.5 rounded-full bg-[var(--border)] text-[var(--text-muted)] text-[9px] flex items-center justify-center font-bold cursor-default select-none">i</span>
        ...
        <span className="text-[var(--text-muted)] text-xs font-mono">
          {isUnlimited ? <>{used} <span className="text-[var(--text-muted)]">used</span></> : `${used} / ${limit}`}
        </span>
      </div>
...
const PLAN_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  free:     { bg: "bg-[var(--toggle-bg)]",    text: "text-[var(--text-sub)]",  border: "border-[var(--border)]" },
  starter:  { bg: "bg-blue-500/10",      text: "text-blue-400",   border: "border-blue-800/40"  },
  pro:      { bg: "bg-violet-500/10",    text: "text-violet-400", border: "border-violet-800/40"},
  business: { bg: "bg-amber-500/10",     text: "text-amber-400",  border: "border-amber-800/40" },
  trial:    { bg: "bg-violet-500/10",    text: "text-violet-400", border: "border-violet-800/40"},
};
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-[var(--text-muted)] mt-1">Manage your plan, track usage, and subscription details.</p>
      </div>
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm overflow-hidden">
              ...
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  <div className="flex items-center gap-3">
                    <h2 className="text-lg font-bold text-[var(--foreground)]">{planLabel} plan</h2>
                ...
                {isPaid && effectivePlan !== "business" && (
                  <button
                    ...
                    className="text-xs font-medium text-[var(--primary)] hover:text-[#0854a0] border border-[#0A66C2]/20 rounded-lg px-3 py-1.5 hover:bg-[var(--primary)]/5 transition-colors"
                  >
                    Adjust plan
                  </button>
                )}
            ...
            <div className="px-6 py-4 space-y-3">
              {isTrial && trialDaysLeft !== null && (
                <div className="flex items-center gap-3 p-3.5 rounded-xl bg-violet-500/10 border border-violet-100">
                  <Clock className="w-4.5 h-4.5 text-violet-400 shrink-0" />
                  <div>
              ...
              {isPaid && renewalDate && (
                <div className="flex items-center gap-3 p-3.5 rounded-xl bg-[var(--card-hover)] border border-[var(--border-sub)]">
                  <RefreshCw className="w-4 h-4 text-[var(--text-muted)] shrink-0" />
                  <div>
                    <p className="text-sm text-[var(--foreground)]">
                      ...
                      {daysUntilRenewal !== null && daysUntilRenewal <= 7 && (
                        <span className="ml-1 text-amber-400 font-medium">({daysUntilRenewal} day{daysUntilRenewal !== 1 ? "s" : ""} left)</span>
                      )}
                    </p>
                  </div>
                </div>
              ...
              {isPaid && !renewalDate && (
                <div className="flex items-center gap-3 p-3.5 rounded-xl bg-emerald-500/10 border border-green-100">
              ...
              {isFree && !isTrial && (
                <div className="flex items-center gap-3 p-3.5 rounded-xl bg-amber-500/10 border border-amber-100">
                  <Sparkles className="w-4 h-4 text-amber-400 shrink-0" />
                  <div>
                    <p className="text-sm font-semibold text-amber-900">You&apos;re on the Free plan</p>
                    <p className="text-xs text-amber-400 mt-0.5">5 posts/month, 2 AI images. Upgrade below for more.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm overflow-hidden">
              <div className="px-6 pt-5 pb-2 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-semibold text-[var(--foreground)]">Plan usage limits</h3>
                </div>
              ...
              <div className="border-t border-[var(--border-sub)] px-6 py-3 flex items-center gap-1.5">
                ...
                <div className="space-y-3">
                  <div className="flex gap-2.5">
                    <input
                      ...
                      className="flex-1 border border-[var(--border)] rounded-lg px-4 py-2.5 text-sm font-mono tracking-wider text-[var(--foreground)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[#0A66C2]/20 focus:border-[#0A66C2] transition-all"
                    />
                    <button
                      onClick={handleRedeem}
                      disabled={redeemLoading || !promoCode.trim()}
                      className="flex items-center gap-2 px-5 py-2.5 bg-[var(--primary)] hover:opacity-90 disabled:opacity-50 text-white text-sm font-medium rounded-lg transition-colors shrink-0"
                    >
            ...
            <div id="upgrade-section" className="space-y-4">
              <details className="group">
                <summary className="flex items-center gap-2 cursor-pointer list-none">
                  <ChevronRight className="w-4 h-4 text-[var(--text-muted)] transition-transform group-open:rotate-90" />
          ...
          {isPaid && ctxSubId && (
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm overflow-hidden">
              <div className="px-6 pt-5 pb-2">
                <h3 className="text-sm font-semibold text-[var(--foreground)]">Cancellation</h3>
              </div>
              <div className="border-t border-[var(--border-sub)]" />
              <div className="px-6 py-4">
                {cancelSuccess ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-[var(--foreground)]">Cancel plan</p>
                    ...
                    {!showCancelConfirm ? (
                      <button
                        ...
                        className="shrink-0 px-4 py-2 rounded-lg text-sm font-medium text-red-400 border border-red-800/40 hover:bg-red-500/10 transition-colors"
                      >
                        Cancel
                      </button>
                    ) : (
                      <div className="flex items-center gap-2 shrink-0">
                        <button
                          ...
                          className="px-3 py-2 rounded-lg text-sm text-[var(--text-sub)] hover:bg-[var(--card-hover)] transition-colors"
                        >
                          Keep plan
                        </button>
                        <button
