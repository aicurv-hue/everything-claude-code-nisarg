  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5 animate-fade-in">
      ...
      {(() => {
        ...
        const dispName: string = seg?.name || (isIndividual ? "Your Name" : "Company Name");
        const dispRole: string = seg?.roleOrIndustry || (isIndividual ? "Add your role in Settings" : "Add your industry in Settings");

        const has = (k: string) => typeof seg?.[k] === "string" && seg[k].trim().length > 0;
        const sectionsDone = [
          has("name") && has("roleOrIndustry") && has("bioOrOffering"),
          has("icp") && has("jtbd"),
          has("pillars") && has("usp"),
            ...
            <div className="flex items-center gap-4">
              {pct < 100 && (
                <Link
                  ...
                  className="hidden sm:flex flex-col items-end gap-1 group"
                  title="Complete your profile in Settings"
                >
                  <div className="flex items-center gap-1.5">
                    <Sparkles className="w-3 h-3 text-[var(--primary)]" />
                    <span className="text-[11px] font-semibold text-[var(--text-sub)] group-hover:text-[var(--foreground)] transition-colors">
                      Profile {pct}% complete
                    </span>
                  </div>
                  <div className="w-[140px] h-[5px] rounded-full" style={{ background: 'var(--progress-bg)' }}>
